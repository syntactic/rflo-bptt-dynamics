...
def parse_args():
    ...
    parser.add_argument("--format", type=str, default="png", choices=["png", "pdf", "svg"], help="Output image format (default: 'png')")
    parser.add_argument("--dpi", type=int, default=300, help="DPI for raster outputs (default: 300)")
    return parser.parse_args()


def generate_all_figures(effector: str, seeds: list[int], results_dir: Path, img_format: str = "png", dpi: int = 300):
    rules = ("BPTT", "RFLO")
    ...
    plot.plot_reach_trajectories(bptt_run["FT"], bptt_run["goal"], bptt_run["direction_idx"], ax=axes[0, 0], title=f"BPTT Reaches (Seed {first_seed})")
    plot.plot_reach_trajectories(rflo_run["FT"], rflo_run["goal"], rflo_run["direction_idx"], ax=axes[0, 1], title=f"RFLO Reaches (Seed {first_seed})")
    ...
    width = 0.35
    axes[0, 2].bar(x - width/2, [per_dir_data["BPTT"][d][0] for d in directions], width, label="BPTT", color="tab:blue")
    axes[0, 2].bar(x + width/2, [per_dir_data["RFLO"][d][0] for d in directions], width, label="RFLO", color="tab:orange")
    axes[0, 2].set_xticks(x)
    axes[0, 2].set_xticklabels(directions)
    axes[0, 2].set_xlabel("Target Direction Index")
    axes[0, 2].set_ylabel("Final Success Rate")
    ...
    all_runs_metrics = [runs[(r, s)]["metrics"] for r in ("BPTT", "RFLO") for s in seeds]
    common_steps = sorted(set.intersection(*(set(step for step, _ in m) for m in all_runs_metrics)))
    ...
    axes[1, 1].plot(common_steps, bptt_succ, marker="o", ms=4, label="BPTT", color="tab:blue")
    axes[1, 1].plot(common_steps, rflo_succ, marker="o", ms=4, label="RFLO", color="tab:orange")
    axes[1, 1].set_xlabel("Training Step")
    axes[1, 1].set_ylabel("Mean Success Rate")
    axes[1, 1].set_ylim(-0.05, 1.05)
    axes[1, 1].set_title("Behavioral Success Rate")
    axes[1, 1].legend()

    bptt_dev = _extract_metric_curve("BPTT", 1)
    rflo_dev = _extract_metric_curve("RFLO", 1)
    axes[1, 2].plot(common_steps, bptt_dev, marker="o", ms=4, label="BPTT", color="tab:blue")
    axes[1, 2].plot(common_steps, rflo_dev, marker="o", ms=4, label="RFLO", color="tab:orange")
    axes[1, 2].set_xlabel("Training Step")
    axes[1, 2].set_ylabel("Mean Path Deviation (m)")
    ...
    pc_act_trajs, pca_act = analysis.process_activation_trajectories(runs, n_components=0.95)
    fig_act_pca, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
    plot.plot_activation_pca(pc_act_trajs[("BPTT", first_seed)], pca=pca_act, title=f"BPTT Hidden Activations (Seed {first_seed})", ax=ax1)
    plot.plot_activation_pca(pc_act_trajs[("RFLO", first_seed)], pca=pca_act, title=f"RFLO Hidden Activations (Seed {first_seed})", ax=ax2)
    ...
    fig_act_dsa = analysis.plot_dsa_heatmap(act_sims, act_labels, title=f"Activation DSA: RFLO vs BPTT ({effector})")
    ...
    pc_weight_trajs, pca_weight = analysis.process_weight_trajectories(runs, bin_size=10, n_components=0.95)
    fig_weight_pca = plot.plot_weight_pca(pc_weight_trajs, pca=pca_weight, title=f"Recurrent Weight Learning Trajectories ({effector})")
    ...
    # 5. Weight DSA Heatmap
    print("Generating Weight DSA Heatmap...")
    weight_systems, weight_labels = [], []
    for rule in rules:
        for seed in seeds:
            weight_systems.append(pc_weight_trajs[(rule, seed)])
            weight_labels.append(f"{rule}-{seed}")
    weight_sims = analysis.run_dsa(weight_systems, n_delays=10, rank=10)
    fig_weight_dsa = analysis.plot_dsa_heatmap(weight_sims, weight_labels, title=f"Weight-Trajectory DSA ({effector})")
    weight_dsa_save = results_dir / f"{effector}_weight_dsa_heatmap.{img_format}"
    fig_weight_dsa.savefig(weight_dsa_save, dpi=dpi)
    print(f"Saved: {weight_dsa_save}")
    plt.close(fig_weight_dsa)
...
def main():
    ...
    generate_all_figures(args.effector, seeds, results_dir, img_format=args.format, dpi=args.dpi)
